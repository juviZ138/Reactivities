     ...
     IEmailSender<User> emailSender, IConfiguration config) : BaseApiController
{
    ...
    public async Task<ActionResult> LoginWithGithub(string code)
    {
        ...
        var tokenReponse = await httpClient
            .PostAsJsonAsync("https://github.com/login/oauth/access_token",
        ...
        var tokenContent = await tokenReponse.Content.ReadFromJsonAsync<GithubTokenResponse>();
        ...
        httpClient.DefaultRequestHeaders.Authorization = new
            System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", tokenContent.AccessToken);
        ...
        if (string.IsNullOrEmpty(user?.Email))
        {
            ...
            if (emailResponse.IsSuccessStatusCode)
            {
                ...
                var primary = emails?
                    .FirstOrDefault(e => e is { Primary: true, Verified: true })?.Email;
    ...
    public async Task<ActionResult> ResendConfirmEmail(string? userId, string? email)
    {
    ...
    {
        var code = await signInManager.UserManager
